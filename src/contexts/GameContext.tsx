      ...
      if (userItemTransfers.length > 0) {
        for (const transfer of userItemTransfers) {
          try {
            ...
            if (error.message && error.message.includes('Limite de 3 itens por tipo atingido')) {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          setGameStats(prev => ({
            ...prev,
            health: u.life_percentage ?? prev.health ?? 100,
            hunger: u.hunger_percentage ?? prev.hunger ?? 100,
            alcoholism: u.alcoholism_percentage ?? prev.alcoholism ?? 0,
            happiness: u.happiness_percentage ?? prev.happiness ?? 100,
            energy: u.energy_percentage ?? prev.energy ?? 100,
            disease: u.disease_percentage ?? prev.disease ?? 0,
          }));
        }
      )
      .subscribe();
