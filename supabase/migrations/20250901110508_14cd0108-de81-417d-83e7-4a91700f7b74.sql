-- Update hunger decrease function to use 20 minutes instead of 5 minutes
...
BEGIN
    ...
    ELSE
        -- Check if 20 minutes have passed since last decrease (1200 seconds)
        IF EXTRACT(EPOCH FROM (now() - hunger_control_record.last_decrease_at)) >= 1200 THEN
            should_decrease := true;
        END IF;
    END IF;
    
    -- Calculate seconds until next decrease
    next_decrease_seconds := GREATEST(0, 1200 - EXTRACT(EPOCH FROM (now() - hunger_control_record.last_decrease_at))::INTEGER);
